...
local function buildPayload(status, leaveTime)
    ...
    return {
        ...
        embeds = {{
            title = "Roblox Player Activity",
            url = profileUrl,
            color = status == "JOIN" and 0x00FF00 or 0xFF0000,
            fields = {
...
local function send(status, leaveTime)
    local res = request({
