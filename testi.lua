...
local autoEnabled = false -- Define autoEnabled early for webhook check
...
-- ✅ URL Wordlist Terbaru
local RANKING_URL               = "https://raw.githubusercontent.com/QuavixAlt/-Q7-Zx-N-3L-8mT2P-EwHkA9cFrXyS/refs/heads/main/words.txt"
...
local function loadCachedData(url, cacheFile, isJson, progressCallback)
    ...
    if isJson then
        ...
        if s then
            ...
            -- Jika JSONDecode gagal, kita asumsikan ini format kata baris demi baris (txt)
            result = {}
            for line in response:gmatch("[^\r\n]+") do
                table.insert(result, {word = line:gsub("%s+", ""), score = 0})
            end
        end
    else
    ...
    if result and CAN_SAVE then pcall(writefile, cacheFile, response) end
    if progressCallback then progressCallback(100) end
    return result
end
...
local function loadMainWordlist(progressFn)
    ...
    for _, entry in ipairs(data) do
        local w = ""
        local score = 0
        if type(entry) == "table" and entry.word then
            w = string.lower(entry.word)
            score = entry.score or 0
        elseif type(entry) == "string" then
            w = string.lower(entry)
        end
...
local function buildIndex()
    ...
    for i = 1, #kataModule do
        ...
        if not wordsByFirstLetter[first] then wordsByFirstLetter[first] = {} end
...
local function updateOverallProgress()
    ...
    if p >= 100 then
        ...
        WindUI:Notify({ Title = "✅ Loading Selesai", Content = #kataModule .. " kata tersedia", Duration = 3 })
    end
end
...
-- SISA KODE (REMOTES, LOGIC, UI, DLL) SESUAI FILE ASLI ANDA
-- [Struktur fungsi ke bawah dipertahankan secara utuh dari file asli...]


-- =========================================================
-- SISA CODE TETAP SAMA (REMOTES, STATE, LOGIC, UI, LOOP)
-- =========================================================
-- (Tempelkan sisa kode asli Anda di bawah ini)
